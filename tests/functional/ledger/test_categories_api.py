...
class TestCategoryViewSet:
    ...
    def test_list_returns_only_current_users_categories(self, client):
        ...
        assert [item["name"] for item in response.data["results"]] == ["Food", "Standalone"]
        assert response.data["results"][0]["children"] == [
            {
                "id": response.data["results"][0]["children"][0]["id"],
                "name": "Snacks",
                "color": None,
                "icon": None,
                "transaction_type": {
    ...
    def test_delete_parent_category_detaches_children_instead_of_hiding_them(self, client):
        user = self._auth(client, username="category_delete_parent")
        expense_type = self._txn_type()
        parent = Category.objects.create(
            name="Parent",
            transaction_type=expense_type,
        ...
        response = client.delete(reverse("ledger:category-detail", args=[parent.id]))

        assert response.status_code == http_client.NO_CONTENT
        parent.refresh_from_db()
        child.refresh_from_db()
        assert parent.deleted_at is not None
        assert child.parent_category is None
        assert child.deleted_at is None

        list_response = client.get(reverse("ledger:category-list"))

        assert list_response.status_code == http_client.OK
        assert [item["name"] for item in list_response.data["results"]] == ["Child"]
        assert list_response.data["results"][0]["children"] == []
